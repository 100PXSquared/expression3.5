...
function PANEL:AddGlobalNode()
	...
	for perm, data in pairs(EXPR_LIB.PERMS) do
		...
		btn.ChangedValue = function(stateBox)
			EXPR_PERMS.SetGlobalPermission(perm, stateBox:GetValue())
		end
	end
end
...
function PANEL:AddGlobalURLNode(oNode)
	...
	btn.ChangedValue = function(stateBox)
		EXPR_PERMS.SetGlobalPermission("URL", stateBox:GetValue())
	end
end
...
function PANEL:AddEntity(entity)
	if entity and not self.entities[entity] then
		...
		for perm, data in pairs(EXPR_LIB.PERMS) do
			...
			btn.ChangedValue = function(stateBox)
				EXPR_PERMS.Set(entity, LocalPlayer(), perm, stateBox:GetValue())
			end
		end
...
function PANEL:AddEntityURLNode(eNode, entity)
	...
	btn.ChangedValue = function(stateBox)
		EXPR_PERMS.Set(entity, LocalPlayer(), "URL", stateBox:GetValue())
	end
end
