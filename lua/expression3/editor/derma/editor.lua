...
function PANEL:_OnKeyCodeTyped(code)
	...
	if control then
		...
		elseif code == KEY_F then
			self.Master.pnlSearch:OpenFindWindow("find")
		elseif code == KEY_H then
			self.Master.pnlSearch:OpenFindWindow("find and replace")
		elseif code == KEY_G then
			self.Master.pnlSearch:OpenFindWindow("go to line")
		elseif code == KEY_UP then
			if shift then
				if self:HasSelection() then
...
function PANEL:MovePosition(caret, offset)
	...
	local row, col = caret.x, caret.y

	if offset > 0 then
		while true do
			local length = #(self.tRows[row]) - col + 2
			if offset < length then
				col = col + offset
				break
...
function PANEL:ScrollCaret()
	...
	if self.Caret.x - self.Scroll.x < 2 then
		self.Scroll.x = self.Caret.x - 2 - Offset
		if self.Scroll.x < 1 then self.Scroll.x = 1 end
	end

	if self.Caret.x - self.Scroll.x > self.Size.x - 2 then
		self.Scroll.x = self.Caret.x - self.Size.x + 2 - Offset
		if self.Scroll.x < 1 then self.Scroll.x = 1 end
	end
...
function PANEL:ExpandAll(tOld)
	...
	if istable(tOld) then
		...
		for line = 1, #self.tRows do
			...
			if istable(Line) and Line.Primary == line then
...
function PANEL:Paint(w, h)
	...
	surface_SetDrawColor(30, 30, 30, 255)
...
function PANEL:DrawRow(Row, LinePos, bForceRepaint)
	...
	draw_SimpleText(tostring(Row), self.Font, self.BookmarkWidth + self.LineNumberWidth, self.FontHeight * LinePos, Color(43, 145, 175), TEXT_ALIGN_RIGHT)
