...
function COMPILER.BuildScript(this, instruction)
	-- This will probably become a separate stage (post interpreter?).
	local outBuffer = {};
	local traceTable = {};
...
function COMPILER.Interpret(this, inst)
	if (not inst) then
		debug.Trace();
		error("Interpreter was asked to interpret a nil instruction.")
	end
	...
	if (not inst.compiled) then
		...
		local fun = this["Interpret_" .. instruction];

		if (not fun) then
			this:Throw(inst.token, "Failed to interpret unknown instruction %s", instruction);
		end
...
function COMPILER.Interpret_ROOT(this, inst, token, data)
	this:writeToBuffer(inst, "\nreturn function(env)\n");
	this:writeToBuffer(inst, "\nsetfenv(1,env)\n");
	...
	if stmts then
		...
		for i = 1, #stmts do
			local r, c, p = this:Interpret(stmts[i]);
			price = price + p;
		end
...
function COMPILER.Interpret_SEQ(this, inst, token, data)
	local stmts = data.stmts;
...
function COMPILER.Interpret_IF(this, inst, token, data)
	this:writeToBuffer(inst, "if (");

	local condition = data.condition;

	local r, c = this:Interpret(condition);
...
function COMPILER.Interpret_ELSEIF(this, inst, token, data)
	this:writeToBuffer(inst, "\nelseif (");

	local condition = data.condition;

	local class, count = this:Interpret(condition);
	...
	if (eif) then
		local _, __, inst4 = this:Interpret(eif);
...
function COMPILER.Interpret_ELSE(this, inst, token, data)
	this:writeToBuffer(inst, "\nelse\n");
...
function COMPILER.Interpret_SERVER(this, inst, token, data)
...
function COMPILER.Interpret_CLIENT(this, inst, token, data)
...
function COMPILER.Interpret_GLOBAL(this, inst, token, data)
	...
	for i = 1, tArgs do
		...
		local r, c, p = this:Interpret(arg);
...
function COMPILER.Interpret_LOCAL(this, inst, token, data)
	local tVars = #data.variables;
	local tArgs = #data.expressions;
...
function COMPILER.Interpret_ASS(this, inst, token, data)
	local price = 1;
	local classes = {};
...
function COMPILER.Interpret_AADD(this, inst, token, data)
	local r, c, p;
	local price = 1;
...
function COMPILER.Interpret_ASUB(this, inst, token, data)
	local r, c, p;
	local price = 1;
	local vt = #data.variables;
	local et = #data.expressions;
...
function COMPILER.Interpret_ADIV(this, inst, token, data)
	local r, c, p;
	local price = 1;
	local vt = #data.variables;
	local et = #data.expressions;
...
function COMPILER.Interpret_AMUL(this, inst, token, data)
	local r, c, p;
	local price = 1;
	local vt = #data.variables;
	local et = #data.expressions;
...
function COMPILER.Interpret_GROUP(this, inst, token, data)

	this:writeToBuffer(inst, "(");

	local r, c, p = this:Interpret(data.expr);
...
function COMPILER.Interpret_TEN(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);

	local expr2 = data.expr2;
	local r2, c2, p2 = this:Interpret(expr2);

	local expr3 = data.expr3;
	local r3, c3, p3 = this:Interpret(expr3);
...
function COMPILER.Interpret_OR(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_AND(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_BXOR(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_BOR(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_BAND(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_EQ(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_NEQ_MUL(this, inst, token, data)
	--(function(value) return operations; end)(value)
...
function COMPILER.Interpret_NEQ(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_LTH(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_LEQ(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_GTH(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_GEQ(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_BSHL(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_BSHR(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_ADD(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_SUB(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_DIV(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_MUL(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_EXP(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_MOD(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_NEG(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_NOT(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_LEN(this, inst, token, data)
	local expr1 = data.expr;
	local r1, c1, p1 = this:Interpret(expr1);
...
function COMPILER.Interpret_DELTA(this, inst, token, data)
	local var = data.var;
...
function COMPILER.Interpret_CHANGED(this, inst, token, data)
	local var = data.var;
...
function COMPILER.Interpret_IOF(this, inst, token, data)
	local r, c, p = this:Interpret(data.expr);
...
function COMPILER.Interpret_CAST(this, inst, token, data)
...
function COMPILER.Interpret_VAR(this, inst, token, data)
...
function COMPILER.Interpret_BOOL(this, inst, token, data)
	if data.value then
...
function COMPILER.Interpret_NUM(this, inst, token, data)
	this:writeToBuffer(inst, data.value);
	return "n", 1, EXPR_MIN;
end

function COMPILER.Interpret_STR(this, inst, token, data)
	this:writeToBuffer(inst, data.value);
	return "s", 1, EXPR_MIN;
end

function COMPILER.Interpret_PTRN(this, inst, token, data)
	this:writeToBuffer(inst, data.value);
	return "_ptr", 1, EXPR_MIN;
end

function COMPILER.Interpret_CLS(this, inst, token, data)
	this:writeToBuffer(inst, "\"" .. data.value .. "\"");
	return "_cls", 1, EXPR_MIN;
end

function COMPILER.Interpret_VOID(this, inst, token)
	this:writeToBuffer(inst, "void");
	return "", 1, EXPR_MIN;
end

function COMPILER.Interpret_COND(this, inst, token, data)
	local expr = data.expr;
	local r, c, p = this:Interpret(expr);
...
function COMPILER.Interpret_NEW(this, inst, token, data)
	local op;
	local ids = {};
	local total = #data.expressions;
...
function COMPILER.Interpret_METH(this, inst, token, data)
	local expressions = data.expressions;

	local expr = expressions[1];
	local mClass, mCount = this:Interpret(expr);
...
function COMPILER.Interpret_CONST(this, inst, token, data)
...
function COMPILER.Interpret_FUNC(this, inst, token, data)
	...
	if (data.library.data == "system") then
		local res, count, prc = hook.Run("Expression3.PostInterpret.System." .. data.name, this, inst, token, data, interpret);
...
function COMPILER.Interpret_LAMBDA(this, inst, token, data)
	this:PushScope();
...
function COMPILER.Interpret_RETURN(this, inst, token, data)
...
function COMPILER.Interpret_BREAK(this, inst, token)
...
function COMPILER.Interpret_CONTINUE(this, inst, token)
	if (not this:GetOption("loop", false)) then
...
function COMPILER.Interpret_DELEGATE(this, inst, token, data)
...
function COMPILER.Interpret_FUNCT(this, inst, token, data)
...
function COMPILER.Interpret_CALL(this, inst, token, data)
	local args = data.expressions;
	local tArgs = #args;

	local expr = args[1];
	local res, count, price = this:Interpret(expr);
	--variables, class
...
function COMPILER.Interpret_GET(this, inst, token, data)
	local expressions = data.expressions;
	local value = expressions[1];
	local vType, vCount, vPrice = this:Interpret(value);
	local index = expressions[2];
	local iType, iCount, iPrice = this:Interpret(index);
...
function COMPILER.Interpret_SET(this, inst, token, data)
	local expressions = data.expressions;

	local value = expressions[1];
	local vType, vCount, p1 = this:Interpret(value);

	local index = expressions[2];
	local iType, iCount, p2 = this:Interpret(index);

	local expr = expressions[3];
	local vExpr, c, p3 = this:Interpret(expr);
...
function COMPILER.Interpret_FOR(this, inst, token, data)
	...
	local tStart, cStart, p1 = this:Interpret(start);
	this:addInstructionToBuffer(inst, start);

	this:writeToBuffer(inst, ",");

	local _end = expressions[2];
	local tEnd, cEnd, p2 = this:Interpret(_end);
	...
	if (step) then
		local tStep, cStep, p3 = this:Interpret(step);
...
function COMPILER.Interpret_WHILE(this, inst, token, data)
	this:writeToBuffer(inst, "\nwhile ");

	local r, c, p = this:Interpret(data.condition);
...
function COMPILER.Interpret_EACH(this, inst, token, data)
	local r, c, p = this:Interpret(data.expr);
...
function COMPILER.Interpret_TRY(this, inst, token, data)
...
function COMPILER.Interpret_INPORT(this, inst, token, data)
...
function COMPILER.Interpret_OUTPORT(this, inst, token, data)
	if (this:GetOption("state") ~= EXPR_SERVER) then
...
function COMPILER.Interpret_INCLUDE(this, inst, token, file_path)
	local script;
	...
	if ok then
		...
		if ok then
			local Interpreter = EXPR_COMPILER.New();

			Interpreter:Initialize(res);

			Interpreter.Interpret_ROOT = Inclucde_ROOT;
			Interpreter.__directives = this.__directives;

			Interpreter.__scope = this.__scope;
			Interpreter.__scopeID = this.__scopeID ;
			Interpreter.__scopeData = this.__scopeData;
			Interpreter.__constructors = this.__constructors;
			Interpreter.__operators = this.__operators;
			Interpreter.__functions = this.__functions;
			Interpreter.__methods = this.__methods;
			Interpreter.__enviroment = this.__enviroment;

			ok, res = Interpreter:Run();
...
function COMPILER.Interpret_CLASS(this, inst, token, data)
	local extends;
...
function COMPILER.Interpret_FIELD(this, inst, token, data)
	local expr = data.expr;
	local type, count, price = this:Interpret(expr);
...
function COMPILER.Interpret_DEF_FIELD(this, inst, token, data)
	local tArgs = #data.expressions;
	local userclass = this:GetOption("userclass");
...
function COMPILER.Interpret_SET_FIELD(this, inst, token, data)

	local info;
	local attribute = data.var.data;
	local expressions = data.expressions;
	local r1, c1, p1 = this:Interpret(expressions[1]);
	local r2, c2, p2 = this:Interpret(expressions[2]);
...
function COMPILER.Interpret_CONSTCLASS(this, inst, token, data)
	this:PushScope();
	this:SetOption("loop", false);
...
function COMPILER.Interpret_SUPCONST(this, inst, token, data)
	...
	local new = COMPILER.Interpret_NEW(this, inst, token, data);
...
function COMPILER.Interpret_DEF_METHOD(this, inst, token, data)
	this:PushScope();
	...
	local _, __, blockprice = this:Interpret(data.block);
...
function COMPILER.Interpret_TOSTR(this, inst, token, data)
	local userclass = this:GetOption("userclass");
...
function COMPILER.Interpret_INTERFACE(this, inst, token, data)
	local extends;
...
function COMPILER.Interpret_INTERFACE_METHOD(this, inst, token, data)
